...
# logdir = parameter_save_path = "/data/userdisk1/zyy/new_data/2024_PolyP_Seg/parameter"
# env = "DDP"
...
class Trainer:
    ...
    def validation_single_gpu(self, val_dataset,):
        ...
        if self.model is not None:
            ...
            self.model.eval()
        val_outputs = []
        infer_times = [] # 存储每个subject的推理时间

        # -------- Warm-up (optional, 避免第一次推理时间偏高) --------
        print("Running warm-up...")
        warmup_iter = next(iter(val_loader))
        warmup_iter = self.before_data_to_device(warmup_iter)
        warmup_iter = self.to_device(warmup_iter)
        with torch.no_grad():
            _ = self.validation_step(warmup_iter)
        ...
        for idx, batch in tqdm(enumerate(val_loader), total=len(val_loader)):
            ...
            with torch.no_grad():
                ...
                print(f"[{idx:03d}] Inference time: {infer_time:.2f} s")
        ...
        print("Average Dice: 1: {}  2: {}  3: {}  4:{}".format(v_sum[0], v_sum[1], v_sum[2], v_sum[3]))
        print("Average Dice: {}  ".format(((v_sum[0] + v_sum[1] + v_sum[2] + v_sum[3])/len(v_sum))))

         # ------------------ 统计平均推理时间 ------------------
        avg_infer_time = np.mean(infer_times)
        print(f"Average inference time per case: {avg_infer_time:.2f} seconds")
        print(f"(Min: {np.min(infer_times):.2f}s, Max: {np.max(infer_times):.2f}s)\n")

        return v_sum, val_outputs, infer_times
